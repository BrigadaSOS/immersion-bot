...
class Goal(commands.Cog):
    ...
    async def set_goal(
        ...
        if not amount > 0:
            return await interaction.response.send_message(
                ephemeral=True, content="Only positive numers allowed."
            )
        ...
        if bool:
            return await interaction.response.send_message(
                ephemeral=True, content="You already set this goal."
            )
        ...
        store.new_goal(
            ...
            media_type.upper(),
            amount,
            (helpers.point_message_converter(media_type.upper(), amount, name))[3],
    ...
    async def set_goal_points(
        ...
        await interaction.response.send_message(
            ephemeral=True,
            content=f"""## Set goal till <t:{int(time.mktime((datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)).timetuple()))}:R>:\n- {amount} points of {media_type}\n\nUse ``/goals`` to view your goals for <t:{int(time.mktime((datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)).timetuple()))}:D>""",
        )

    @app_commands.command(
        name="goals", description=f"See your immersion log goal overview."
    )
    async def goals(self, interaction: discord.Interaction):
        ...
        async def goals_row(
            discord_user_id, media_type, amount, text, created_at, frequency
        ):
            return (
                f"""- {amount} {helpers.media_type_format(media_type.value)} {text}"""
            )

        day_start = interaction.created_at.replace(
        ...
        recent_logs = store.get_recent_goal_alike_logs(
            interaction.user.id, (day_start, now)
        )
        print(recent_logs)
        goals_description = []
            ...
            for points_row in point_goals:
                if recent_logs:
                    if points_row.media_type in rl_media_type_l:
                        ...
                        goals_description.append(
                            f"""- {"~~" + str(sum(points)) + "/" + str(points_row.amount) + " points " + points_row.text + (" (" + points_row.freq + ") " if points_row.freq != None else "") + "~~" if sum(points) >= points_row.amount else str(sum(points)) + "/" + str(points_row.amount) + " points " + points_row.text + (" (" + points_row.freq + ") " if points_row.freq != None else "")}"""
                        )
                        continue
                    else:
                        ...
                        if points_row.media_type.value == "ANYTHING":
                            ...
                            goals_description.append(
                                f"""- {"~~" + "0" + "/" + str(points_row.amount) + " points " + points_row.text + (" (" + points_row.freq + ") " if points_row.freq != None else "") + "~~" if sum(points) >= points_row.amount else "0" + "/" + str(points_row.amount) + " points " + points_row.text + (" (" + points_row.freq + ") " if points_row.freq != None else "")}"""
                            )
                            break
                else:
                    goals_description.append(
                        f"""- 0/{str(points_row.amount)} points {points_row.text} {(" (" + points_row.freq + ") " if points_row.freq != None else "")} """
                    )
                    continue
